...
namespace main_application
{
    public partial class Form1 : Form
    {
        ...
        public Mutex Auth_status_mutex = new Mutex(); //мьютексы для управления синхронизацией потоков
            ...
            while (serialPort1.IsOpen)
            {
                ...
                if (bytestoread > 0)
                {
                    try
                    {
                        ...
                        //мьютекс занят, поток ждет его освобождения
                        ...
                        //освобождение мьютекса, поток продолжает работу
        ...
        ********************************************************/
        ...
        public void FrameReceivedLogout(DefaultFrame frame)
        {
            ...
            //1 арг - метод, который будет выполняться в результате завершения работы асинхронного делегата
            //2 арг произвольный объект, с помощью которого мы можем передать дополнительную информацию в 
            //метод завершения, указанный в предыдущем параметре.- 
